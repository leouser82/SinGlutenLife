...
/** Consulta corta: etiqueta publicada o nombre con gluten, en cualquier ciudad. */
export function osmFastGfQuery(lat, lon, km) {
  const radius = Math.round(Math.min(km, 30) * 1000)
  return `[out:json][timeout:8];
(
  node["diet:gluten_free"](around:${radius},${lat},${lon});
  way["diet:gluten_free"](around:${radius},${lat},${lon});
  node["name"~"gluten",i](around:${radius},${lat},${lon});
);
out tags center;`
}
